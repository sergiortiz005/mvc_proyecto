    ...
    <div class="container">
        ...
        <div class="usuario-info">
            ...
            <?php if (!empty($datos['usuario']->foto_perfil)): ?>
                <img src="<?= RUTA_URL . '/public/img/perfiles/' . $datos['usuario']->foto_perfil ?>" alt="Foto perfil" style="max-width:150px; margin-top:10px;">
            <?php endif; ?>
        </div>
        ...
        <h3>Editar datos</h3>
        <form action="<?= RUTA_URL ?>/usuario/editarMiCuenta" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?= $datos['usuario']->id ?>" />

            <label for="nombre">Nombre:</label>
            <input type="text" id="nombre" name="nombre" value="<?= htmlspecialchars($datos['usuario']->nombre) ?>" required />

            <label for="apellido">Apellido:</label>
            <input type="text" id="apellido" name="apellido" value="<?= htmlspecialchars($datos['usuario']->apellido) ?>" required />

            <label for="telefono">Teléfono:</label>
            <input type="text" id="telefono" name="telefono" value="<?= htmlspecialchars($datos['usuario']->telefono) ?>" required />
